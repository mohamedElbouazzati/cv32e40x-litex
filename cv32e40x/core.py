...
# CPUCompressed = [("compressed_valid", 1, DIR_M_TO_S),
#                                 ("compressed_ready", 1, DIR_S_TO_M),
#                                 ("compressed_req", 1, DIR_M_TO_S),
#                                 ("compressed_resp", 1, DIR_S_TO_M)]

# CPUIssue = [("issue_valid", 1, DIR_M_TO_S),
#                                 ("issue_ready", 1, DIR_S_TO_M),
#                                 ("issue_req", 1, DIR_M_TO_S),
#                                 ("issue_resp", 1, DIR_S_TO_M)]

# class CPUCommit(Record):
#     def __init__(self):
#         Record.__init__(self, [("commit_valid", 1, DIR_M_TO_S),
#                                 ("commit", 1, DIR_M_TO_S)])

# class CPUMem(Record):
#     def __init__(self):
#         Record.__init__(self, [("mem_valid", 1, DIR_S_TO_M),
#                                 ("mem_ready", 1, DIR_M_TO_S),
#                                 ("mem_req", 1, DIR_S_TO_M),
#                                 ("mem_resp", 1, DIR_M_TO_S)])

# class CPUMemResult(Record):
#     def __init__(self):
#         Record.__init__(self, [("mem_result_valid", 1, DIR_S_TO_M),
#                                 ("mem_result", 1, DIR_M_TO_S)])

# class CPUResult(Record):
#     def __init__(self):
#         Record.__init__(self, [("result_valid", 1, DIR_M_TO_S),
#                                 ("result_ready", 1, DIR_S_TO_M),
#                                 ("result", 1, DIR_M_TO_S)])
...
def add_manifest_sources(platform, manifest):
    basedir = get_data_mod("cpu", "cv32e40x").data_location
...
class CV32E40X(CPU):
    ...
    def __init__(self, platform, variant="standard"):
        ...
        #eXtension interface 
        # self.xif_compressed_litex = Record(CPUCompressed)
        # self.xif_issue_litex = Record(CPUIssue)
        # self.xif_commit_litex = CPUCommit()
        # self.xif_mem_litex = CPUMem()
        # self.xif_mem_result_litex= CPUMemResult()
        # self.xif_result_litex= CPUResult
        ...
        self.cpu_params = dict(
            ...
            #eXtension:
            # o_xif_compressed_if = self.xif_compressed_litex,
            # o_xif_issue_if = self.xif_issue_litex
            # o_xif_commit_if = self.xif_commit_litex,
            # o_xif_mem_if = self.xif_mem_litex,
            # o_xif_mem_result_if=  self.xif_mem_result_litex,
            # o_xif_result_if    = self.xif_result_litex,
            #extra signals:
        )

        # Add Verilog sources.
        add_manifest_sources(platform, 'cv32e40x_manifest.flist')
    ...
    def do_finalize(self):
        ...
        self.specials += Instance("cv32e40x_core", **self.cpu_params)
